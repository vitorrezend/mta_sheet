#!/usr/bin/env bash
# ==============================================================================
# MTA Sheet - Backup SQLite Direto (.backup - Rápido / Online)
# ==============================================================================
...
DEST_FILE="${DEST_DIR}/${DB_FILENAME}_${TIMESTAMP}.db"

echo "⏳ Iniciando backup direto online de $DB_SOURCE_PATH..."
sqlite3 "$DB_SOURCE_PATH" ".backup '$DEST_FILE'"

# Verificação de integridade
INTEGRIDADE=$(sqlite3 "$DEST_FILE" "PRAGMA integrity_check;")
if [ "$INTEGRIDADE" = "ok" ]; then
    TAMANHO=$(du -h "$DEST_FILE" | cut -f1)
    echo "✅ Backup direto concluído com SUCESSO!"
